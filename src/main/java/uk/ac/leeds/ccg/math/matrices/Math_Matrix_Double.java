...
/**
 * For processing matrices holding double numbers.
 *
 * @author Andy Turner
 * @version 1.0
 */
public class Math_Matrix_Double {
     ...
     * matrix.
     */
    protected Math_Matrix_Double mt;
     ...
     */
    protected Math_Matrix_Double ref;
     ...
     */
    protected Math_Matrix_Double rref;
     ...
     */
    public Math_Matrix_Double(double[][] m) {
        int nr = m.length;
        int nc = m[0].length;
        this.rows = new double[nr][nc];
        this.cols = new double[nc][nr];
        for (int r = 0; r < nr; r++) {
            for (int c = 0; c < nc; c++) {
                rows[r][c] = m[r][c];
                cols[c][r] = m[r][c];
     ...
     */
    protected Math_Matrix_Double(double[][] rows, double[][] cols) {
        this.rows = rows;
        this.cols = cols;
    }
    ...
    public boolean equals(Object o) {
        if (o instanceof Math_Matrix_Double d) {
            return equals(d);
        }
        return false;
    }
     ...
     */
    public boolean equals(Math_Matrix_Double m) {
        if (rows.length == m.rows.length && cols.length == m.cols.length) {
            for (int r = 0; r < rows.length; r++) {
                for (int c = 0; c < cols.length; c++) {
                    if (this.rows[r][c] != m.rows[r][c]) {
                        return false;
                    }
                }
            }
     ...
     */
    public Math_Matrix_Double multiply(Math_Matrix_Double m) {
        Math_Matrix_Double r = null;
        if (cols.length == m.rows.length) {
            double[][] rrows = new double[rows.length][m.cols.length];
            double[][] rcols = new double[m.cols.length][rows.length];
            for (int row = 0; row < rows.length; row++) {
                for (int col = 0; col < m.cols.length; col++) {
                    double v = 0d;
                    for (int i = 0; i < m.rows.length; i++) {
                        v += rows[row][i] * m.rows[i][col];
                    }
                    rrows[row][col] = v;
                    rcols[col][row] = v;
                }
            }
            r = new Math_Matrix_Double(rrows, rcols);
        }
        return r;
    }
     ...
     */
    public Math_Matrix_Double multiply(double s) {
        Math_Matrix_Double r;
        double[][] rrows = new double[rows.length][cols.length];
        double[][] rcols = new double[cols.length][rows.length];
        for (int row = 0; row < rows.length; row++) {
            for (int col = 0; col < cols.length; col++) {
                double v = rows[row][col] * s;
                rrows[row][col] = v;
                rcols[col][row] = v;
            }
        }
     ...
     */
    public Math_Matrix_Double add(Math_Matrix_Double m) {
        Math_Matrix_Double r = null;
        if (cols.length == m.cols.length) {
            if (rows.length == m.rows.length) {
                ...
                for (int row = 0; row < rows.length; row++) {
                    for (int col = 0; col < cols.length; col++) {
                        double v = rows[row][col] + m.rows[row][col];
                        rrows[row][col] = v;
                        rcols[col][row] = v;
                    }
                }
        ...
        switch (m.length) {
            ...
            default:
                ...
                for (int i = 0; i < m.length; i++) {
                    double sr = m[0][i] * (getDeterminant(
                            getSubMatrix(m, i)));
                    if (i % 2 == 0) {
                        r = r + (sr);
                    } else {
    ...
    private double[][] getSubMatrix(double[][] m, int col) {
        int len = m.length;
        int len2 = len - 1;
        int col2 = col + 1;
        int r = 0;
        double[][] res = new double[len2][len2];
        for (int i = 1; i < len; i++) {
            int c = 0;
            for (int j = 0; j < col; j++) {
                res[r][c] = m[i][j];
     ...
     */
    public static Math_Matrix_Double getIdentityMatrix(int size) {
        Math_Matrix_Double r;
        double[][] rrows = getMatrix(size, size, 0d);
        double[][] rcols = getMatrix(size, size, 0d);
     ...
     * equal to v.
     */
    public static double[][] getMatrix(int rows, int cols, double v) {
    ...
    public double[][] getCols() {
        double[][] m = new double[cols.length][rows.length];
        for (int c = 0; c < cols.length; c++) {
     ...
     */
    public Math_Matrix_Double getTranspose() {
        if (mt == null) {
            double[][] rrows = new double[cols.length][rows.length];
            double[][] rcols = new double[rows.length][cols.length];
            for (int row = 0; row < rows.length; row++) {
                for (int col = 0; col < cols.length; col++) {
            ...
            mt = new Math_Matrix_Double(rrows, rcols);
            mt.mt = this;
        }
        return mt;
    }
     ...
     * @return {@code true} iff the row has all zero values.
     */
    public static boolean isZeroRow(double[][] m, int row) {
     ...
     * @return {@code true} iff the column has all zero values.
     */
    public boolean isZeroCol(double[][] m, int col) {
        for (double[] row : m) {
            if (row[col] == 0d) {
                return false;
            }
    ...
    public int getRank() {
        int r = 0;
        Math_Matrix_Double m = getReducedRowEchelonForm();
        for (int i = 0; i < m.rows.length; i++) {
            if (!m.isZeroRow(i)) {
                r++;
            }
     ...
     */
    private void swapRows(double[][] m, int i, int r) {
        double[] mr0 = m[r];
        m[r] = m[i];
        m[i] = mr0;
    }
     ...
     */
    public Math_Matrix_Double getRowEchelonForm() {
        if (ref == null) {
            double[][] m = getRows();
            int h = 0;
            ...
            for (h = rows.length - 1; h >= 0; h--) {
                if (!Math_Matrix_Double.isZeroRow(m, h)) {
                    int i = 0;
                    double v = 0d;
                    for (int col = 0; col < cols.length; col++) {
                        if (m[h][col] != 0d) {
     ...
     */
    public Math_Matrix_Double getReducedRowEchelonForm() {
        if (rref == null) {
            double[][] m = getRowEchelonForm().getRows();
     ...
     */
    public static int getColIndexOfSecondNonZeroValue(double[][] m, int row) {
     ...
     */
    public static int getMaxRowIndex(double[][] m, int col, int minrow, int maxrow) {
        int r = minrow;
        double max = m[minrow][col];
     ...
     * @return The column index for the column in row with the largest value.
     */
    public static int getMaxColumnIndex(double[][] m, int row) {
        int c = 0;
        double max = m[row][0];
        for (int col = 0; col < m[0].length; col++) {
            if (m[row][col] > max) {
